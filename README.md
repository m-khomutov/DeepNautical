...
В каталоге находится файл **example.cpp**. Это простейший пример использования функционала библиотеки.
Пример собирается командой
```
g++ -o example example.cpp -I. ../build/kformat/lib/libkformat.a
```
Сборка должна производиться в текущем каталоге.
Пример запускается с одним аргументом - путь к файлу jpeg.
Открывает сокет tcp и слушает входные запросы на порт 5555.
Параллельно не работает. Обслуживает запросы последовательно (пример простейший, как было указано).
Отправляет программе просмотра содержимое файла jpeg с периодом 40 мс.
