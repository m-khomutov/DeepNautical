...
Используется не совсем стандартный протокол **Flash Video**. Имеются сделующие несоответствия стандарту:
* Поддержка кодека JPEG. Стандарт определяет как ` JPEG (currently unused)`
* timestamp передается как 4-х байтная переменная **float**. Определение стандарта
```
Timestamp UI24 
