...
В каталоге **examples** находятся файлы **example_frame.cpp** и **example_proto.cpp**.
Это простейшие примеры использования функционала библиотеки.

Примеры собираются командой
```
g++ -o example_frame example_frame.cpp -I.. ../../build/kformat/lib/libkformat.a
```

и

```
g++ -o example_proto example_proto.cpp -I.. ../../build/kformat/lib/libkformat.a
```

Сборка должна производиться в каталоге **examples**.

Пример **example_proto** запускается с одним аргументом - путь к файлу jpeg.
Примеры открывают сокет tcp и слушают входные запросы на порт 5555.

Параллельно не работают. Обслуживают запросы последовательно (примеры простейшие, как было указано).

example_proto отправляет программе просмотра содержимое файла jpeg с периодом 40 мс.

example_frame отправляет программе просмотра сгенерированное изображение красной полосы, двигающейся по экрану.
