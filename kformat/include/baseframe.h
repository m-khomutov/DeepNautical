...
public:
    ...
    */
     ...
     */
    float is_duration_passed();
    /*!
       \brief is_updated возвращает флаг, показывающий что буфер был обновлен
       \return флаг
     */
    bool is_updated()
    {
...
protected:
    ...
    //! размеры кадра (ширина, высота)
    NUtils::TGeometry geometry_;
    //! длительность кадра (в миллисекундах)
    std::chrono::milliseconds duration_;
    //! метка времени, следящая за превышением длительности видеокадра
    time_point_t last_ts_;
    //! буфер хранения пиксельной информации кадра (по количеству задействованных точек обзора)
    image rgb_buffer_;
    //! флаг, показывающий что буфер был обновлен
    bool updated_ {false};
