/*!
     \file: utils.h
     \author: mkh
     \date 26 января 2023 г.
     \brief Заголовочный файл вспомогательных классов и функций.

     Данный файл содержит в себе объявления вспомогательных классов и функций.
 */
#ifndef UTILS_H
#define UTILS_H

#include "json.h"

#include <cstddef>
#include <cstdio>
#include <cstdint>
#include <cstring>
#include <dirent.h>
#include <sys/time.h>
extern "C"
{
 #include <jpeglib.h>
 #include <jerror.h>
}
#include <cstdlib>
#include <functional>
#include <iostream>
#include <string>
#include <iterator>
#include <sstream>
#include <map>
#include <memory>
#include <mutex>
#include <set>
#include <thread>
#include <vector>

namespace utils
{
struct winsize
{
    int width = 800;
    int height = 600;

    winsize() = default;
    winsize( int w, int h )
        : width( w )
        , height( h )
    {}
    winsize( const char * from )
    {
        char *ptr;
        int w = strtol( from, &ptr, 10 );
        if( w && ptr )
        {
            int h = strtol( ptr + 1, nullptr, 10 );
            if( h )
            {
                width = w;
                height = h;
            }
        }
    }
};


struct scenecfg
{
    std::string name;
    winsize size;
    int x = 0;
    int y = 0;
    js::object specification;

    scenecfg() = default;
    scenecfg( const std::string &n, const js::object &obj )
        : name( n )
        , size( obj["size"]["width"].toInt(), obj["size"]["height"].toInt() )
        , x( obj["position"]["x"].toInt() )
        , y( obj["position"]["y"].toInt() )
        , specification( obj )
    {}

    bool operator <( const scenecfg &other ) const
    {
        if( x == other.x )
        {
            return y < other.y;
        }
        return x < other.x;
    }
};
using set_scenecfg_t = std::set< scenecfg >;

template< typename T >
class threadsafe {
public:
    class guard {
    public:
        guard( guard && rhs )
            : m_( std::move( rhs.m_ ) )
            , value_( rhs.value_ )
        {}

        T& operator *()
        {
            return *value_;
        }

        T* operator ->()
        {
            return value_;
        }

    private:
        guard( std::mutex *m, T *v )
            : m_( m, []( std::mutex *m_p ){ m_p->unlock(); } )
            , value_( v )
        {
            m_->lock();
        }

    private:
        std::shared_ptr< std::mutex > m_;
        T *value_;
        friend class threadsafe;
    };

public:
    threadsafe()
        : value_( new T )
    {}
    threadsafe( const threadsafe& orig ) = delete;
    threadsafe &operator =( const threadsafe& orig ) = delete;

    guard value()
    {
        return guard( &mutex_, value_.get() );
    }

private:
    std::mutex mutex_;
    std::unique_ptr< T > value_;
};


template< typename T >
class scopedthread
{
 public:
     scopedthread() = default;
     explicit scopedthread( T *obj )
         : object_( obj )
         , t_( [this](){
             try {
                 object_->start_listening_network();
             }
             catch ( const std::exception &e ) {
                 *exception_.value() = e.what();
             }
           })
     {}
     scopedthread( scopedthread const &orig ) = delete;
     scopedthread &operator =( scopedthread const &rhs ) = delete;

     ~scopedthread()
     {
         if( object_ )
         {
             object_->finish();
             if( t_.joinable() )
             {
                 t_.join();
             }
         }
     }

     std::string exception()
     {
         return *exception_.value();
     }

private:
    T *object_ { nullptr };
    std::thread t_;
    threadsafe< std::string > exception_;
};

class settings
{
public:
    settings() = default;
    settings( int argc, char * argv[] );
    settings( settings const &orig) = delete;

    settings &operator =( settings const &rhs ) = delete;

    class var
    {
    public:
        var() = default;
        var( int v );
        var( std::string v );
        var( winsize const &v );
        var( set_scenecfg_t const &v );

        operator int() const;
        operator std::string() const;
        operator winsize() const;
        operator set_scenecfg_t() const;

    private:
        int i_value_;
        std::string s_value_;
        winsize wsz_value_;
        set_scenecfg_t ssc_value_;
    };
    using settings_t = std::map< std::string, var >;
    
    var &operator [](char const *key) const;
    
private:
    static settings_t fields_;

private:
    void f_read_file( char const *name );
    void f_read_jsfile( char const *name );
};

struct frame
{
    enum class color { Monochrome = 1, RGB = 3 };

    std::vector< uint8_t > pixels;
    winsize window;
    color channels { color::RGB };
    uint64_t timestamp { std::numeric_limits< uint64_t >::max() };

    frame() = default;
    frame( winsize const &win )
        : window( win )
    {}
};


class jpegcodec {
public:
    class error: public std::runtime_error {
    public:
        error(const std::string & what);
    };

    jpegcodec();
    jpegcodec( const jpegcodec& orig ) = delete;
    ~jpegcodec();

    jpegcodec &operator =( const jpegcodec& orig ) = delete;
    
    bool decode( uint8_t const *data, size_t size, frame *fr );
    bool decode( char const *filename, frame *fr );

private:
    jpeg_decompress_struct cinfo_;
    jpeg_error_mgr jerr_;
};

bool file_exists( char const *filename );
void read_config( char const *fname, std::function< void( const std::string& ) > foo );
bool str2key( const std::string &s, std::pair< std::string, std::string > *rc );

template< typename F >
void parsedir( const std::string &path, char const *filter, F &&func )
{
    DIR *dir;
    struct dirent *entry;
    if( !(dir = opendir( path.c_str() ) ) )
        throw std::runtime_error( path + std::string(" error: ") + std::string(strerror( errno ) ) );

    while( (entry = readdir(dir))  )
    {
        if( entry->d_name[0] != '.' && entry->d_type == DT_REG && strstr( entry->d_name, filter ) )
        {
            func( entry->d_name );
        }
    }
    closedir( dir );
}

template< typename T >
bool str2array( const std::string &s, T *rc )
{
    std::istringstream iss(s);
    std::vector< std::string > v( (std::istream_iterator<std::string>(iss)),
                                   std::istream_iterator<std::string>() );
    if( v.size() == sizeof(T) / sizeof(float) )
    {
        for( size_t i(0); i < v.size(); ++i )
        {
            try
            {
                (*rc)[i] = std::stof(v[i]);
            }
            catch( const std::logic_error & err )
            {
                std::cerr << __PRETTY_FUNCTION__ << " error: " << err.what() <<std::endl;
                return false;
            }
        }
        return true;
    }
    return false;
}

inline uint64_t now()
{
    timespec ts;
    clock_gettime( CLOCK_REALTIME, &ts );

    return ts.tv_sec * 1000000000 + ts.tv_nsec;
}

}  // namespace utils

#endif /* UTILS_H */
